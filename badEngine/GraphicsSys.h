...
#define BAD_RENDERER_EXCEPTION(type,note) BadException(__FILE__, __LINE__,type,note)
	...
	public:
		...
		void init_from_config(const nlohmann::json& windowConfig);
		...
		//returns false on failure, call SDL_GetError
		bool draw(SDL_Texture* texture, const Sequence<std::pair<AABB, AABB>>& list)const noexcept;

		//draws full texture 
		bool draw(SDL_Texture* texture)const noexcept;


		SDL_Window* get_window_test() {
			return mWindow.get();
		}
		SDL_Renderer* get_render_test() {
