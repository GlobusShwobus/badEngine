...
bad::bString bad::validate_json_file(const char* path, expected_file_type type) noexcept
{
	// phase 1: test basic file info
	if (!path)
		return bString::failure("Null path provided");

	std::filesystem::path p(path);

	if (!std::filesystem::exists(p))
		return bString::failure("File does not exist: " + std::string(path));

	if (!std::filesystem::is_regular_file(p))
		return bString::failure("Path is not a regular file: " + std::string(path));

	if (std::filesystem::file_size(p) == 0)
		return bString::failure("File is empty: " + std::string(path));
	...
	try {
		...
		if (!in.is_open())
			return bString::failure("Cannot open file: " + std::string(path));
		...
		case expected_file_type::WINDOW_JSON:
			...
			break;
		default:
			return bString::failure("Unknown file type specified");
		}

	}
	catch (const nlohmann::json::parse_error& e) {
		return bString::failure(
		...
		return bString::failure("Unknown error while parsing JSON");
	}

	return bString::success();
}

void bad::validate_details::validate_texture_manifest(const nlohmann::json& manifest, const char* key)
{
...
void bad::validate_details::validate_window_manifest(const nlohmann::json& manifest, const char* key)
{
