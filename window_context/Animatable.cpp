...
namespace badWindow
{
	Clip make_clip(float texture_w, float texture_h, const SDL_FRect& first_frame, uint16_t frame_count, float frame_duration, bool loops)
	{
		if (frame_count == 0)
	...
	AnimationPlayer::AnimationPlayer(SDL_Texture* const texture)
		:mSprite(texture), mTimer(0), mCurrentFrame(0)
	{
	}

	AnimationPlayer::AnimationPlayer(AnimationPlayer&& rhs)noexcept
	...
	AnimationPlayer& AnimationPlayer::operator=(AnimationPlayer&& rhs)noexcept
	{
		if (this != &rhs) {
	...
	void AnimationPlayer::play(AnimID id)
	{
