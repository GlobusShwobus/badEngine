...
namespace badWindow
{
	...
	public:
		...
		void draw_filled_AABB(const AABB& aabb, Color color)const noexcept;

		// renders a line on the screen with a given color
		void draw_line(float x1, float y1, float x2, float y2, Color color)const noexcept;
		void draw_line(const badCore::float2& start, const badCore::float2& end, Color color)const noexcept;
		...
		void draw_closed_model(std::span<const float2> model, const badCore::Mat3& transform, Color color)const noexcept;
